...
`libclang_rt.profile_coverage.a`. Copy its `.syms` companion to the canonical
`.syms` name as well.
...
`LLVM_PROFILE_DENSE=1` to opt into the previous dense serialization and `%m`
online-merging behavior.
