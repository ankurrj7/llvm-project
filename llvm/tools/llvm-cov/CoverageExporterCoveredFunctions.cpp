...
struct StructuralSegment {
  ...
  bool PreserveDuplicatePoint = false;
};
...
void appendCounterPointsInRange(
    std::vector<StructuralSegment> &Segments,
    ArrayRef<StructuralSegment> CounterPoints, LineColPair Start,
    LineColPair End, const StructuralRegion &Owner) {
  if (Owner.HasMultipleCounters)
    return;
  auto Point = llvm::lower_bound(
      CounterPoints, Start,
      [](const StructuralSegment &Segment, LineColPair Loc) {
        return Segment.Start < Loc;
      });
  while (Point != CounterPoints.end() && Point->Start < End) {
    if (!Point->HasMultipleCounters &&
        Point->StaticCounter == Owner.StaticCounter) {
...
buildNestedStructuralSegments(
    ArrayRef<StructuralRegion> Regions,
    ArrayRef<StructuralSegment> ZeroWidthCounterPoints) {
  ...
  while (StartIndex != Regions.size() || !Active.empty()) {
    ...
    if (PreviousLoc && *PreviousLoc < *EventLoc && !Active.empty()) {
      ...
      if (Owner.Kind == CounterMappingRegion::CodeRegion) {
        // A gap or skipped region can end before the next executable token,
        // including in leading whitespace. Do not resume an enclosing code
        // region at that rendering-only boundary. Wait until an actual code
        // region starts before emitting executable coverage again.
        if (LastOwner || Owner.Start == *PreviousLoc) {
          if (LastOwner && haveSameStaticCounter(Regions[*LastOwner], Owner) &&
            ...
            Segments.push_back(
                {*PreviousLoc, *EventLoc, Owner.StaticCounter,
                 Owner.ExecutionCount, Owner.HasMultipleCounters});
          LastOwner = OwnerIndex;
        } else
          appendCounterPointsInRange(Segments, ZeroWidthCounterPoints,
                                     *PreviousLoc, *EventLoc, Owner);
      } else {
        LastOwner.reset();
      }
    }
...
buildStructuralSegments(std::vector<StructuralRegion> Regions) {
  ...
  // executable segment. Also retain zero-width code regions when they are the
  // only executable mapping inside a rendering-only gap (for example around
  // setjmp); they cannot safely be widened to surrounding source. Using the
  // original start location preserves stable identity without reintroducing
  // overlapping ranges.
  ...
  std::vector<StructuralSegment> ZeroWidthCounterPoints;
  for (const StructuralRegion &Region : Regions) {
    if (Region.Kind != CounterMappingRegion::CodeRegion)
      continue;
    StructuralSegment Point{Region.Start, Region.Start, Region.StaticCounter,
                            Region.ExecutionCount,
                            Region.HasMultipleCounters, true};
    FallbackCounterPoints.push_back(Point);
    if (Region.Start == Region.End && !Region.IsExpansionStart) {
      Point.PreserveDuplicatePoint = false;
      ZeroWidthCounterPoints.push_back(Point);
    }
  }

  auto PreserveFallbackCounterPoints =
      [&](std::vector<StructuralSegment> Segments) {
    if (Segments.empty())
  ...
  if (auto Segments =
          buildNestedStructuralSegments(Combined, ZeroWidthCounterPoints)) {
    return PreserveFallbackCounterPoints(std::move(*Segments));
  }
  ...
  while (StartIndex != Combined.size() || EndIndex != Ends.size()) {
    ...
    if (!LastOwner && Owner.Start != Loc) {
      appendCounterPointsInRange(Segments, ZeroWidthCounterPoints, Loc,
                                 *NextLoc, Owner);
      continue;
    }
...
void retainOneSegmentPerCounter(std::vector<StructuralSegment> &Segments) {
  ...
  llvm::erase_if(Segments, [&](const StructuralSegment &Segment) {
    ...
    // A zero-width marker retained inside an otherwise ordinary function is
    // deduplicated like a positive-width segment.
    if (Segment.Start == Segment.End && Segment.PreserveDuplicatePoint)
      return false;
...
class CoverageExporterCoveredFunctions::Implementation {
  ...
  Expected<bool> renderSegmentFunction(StringRef RawFunctionName,
    ...
    std::set<std::pair<unsigned, LineColPair>> ExpansionStarts;
    for (unsigned Ordinal = 0; Ordinal != Function.CountedRegions.size();
      ...
      if (Region.Kind == CounterMappingRegion::ExpansionRegion) {
        ...
        ExpansionStarts.insert({Region.FileID, Region.startLoc()});
        continue;
      }
      if (Region.Kind != CounterMappingRegion::CodeRegion &&
    ...
    for (FunctionFileRegions &File : Files) {
      for (StructuralRegion &Region : File.Regions) {
        if (Region.Start == Region.End &&
            ExpansionStarts.count({File.FileID, Region.Start}))
          Region.IsExpansionStart = true;
      }
    }
