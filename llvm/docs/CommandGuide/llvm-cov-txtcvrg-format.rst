...
region supplies the count. Gap and skipped regions separate executable runs,
but are source-rendering metadata rather than executable coverage. Their end
locations therefore do not create kind ``1`` starts; normalization waits for
an actual code region before resuming executable output.
...
A mapping can contain a zero-width code counter, for example around a
``setjmp`` expansion. When it is the only executable mapping inside a
rendering-only gap, format 4 preserves it as a kind ``1`` point instead of
manufacturing an executable interval around it. A macro-generated function can
also have coverage counters but no positive-width executable interval after
code-region and macro normalization. Removing that function would remove the
only evidence that it ran, so format 4 emits one kind ``1`` point record for
each of its raw code counters:
