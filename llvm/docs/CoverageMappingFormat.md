...
A mapping region stores the {ref}`source code range` that is covered by this region,
the {ref}`file id <coverage file id>`, the {ref}`coverage mapping counter` and
the region's kind.
There are several kinds of mapping regions:

* Code regions associate portions of source code and {ref}`coverage mapping counters <coverage mapping counters>`.
  They make up the majority of the mapping regions. They are used
  ...
  <pre class='highlight' style='line-height:initial;'><span>int main(int argc, const char *argv[]) </span><span style='background-color:#4A789C'>{    </span> <span class='c1'>// Code Region from 1:40 to 9:2</span>
...
* Skipped regions are used to represent source ranges that were skipped
  ...
  {ref}`coverage mapping counters`, as the frontend knows that they are never
  ...
  <pre class='highlight' style='line-height:initial;'><span>int main() </span><span style='background-color:#4A789C'>{               </span> <span class='c1'>// Code Region from 1:12 to 6:2</span>
...
* Expansion regions are used to represent Clang's macro expansions. They
  ...
  expanded file id. They don't associate with {ref}`coverage mapping counters`,
  ...
  <pre class='highlight' style='line-height:initial;'><span>int func(int x) </span><span style='background-color:#4A789C'>{                             </span>
...
* Branch regions associate instrumentable branch conditions in the source code
  with a {ref}`coverage mapping counter` to track how many times an individual
  condition evaluated to 'true' and another {ref}`coverage mapping counter` to
  ...
  <pre class='highlight' style='line-height:initial;'><span>int func(int x, int y) {
...
<pre class='highlight' style='line-height:initial;'><span>void func(const char *str) </span><span style='background-color:#4A789C'>{        </span> <span class='c1'>// Code Region from 1:28 to 6:2 with file id 0</span>
...
<pre class='highlight' style='line-height:initial;'><span>int main(int argc, const char *argv[]) </span><span style='background-color:#4A789C'>{   </span> <span class='c1'>// Region's counter is a reference to the profile counter #0</span>
<span style='background-color:#4A789C'>                                           </span>
...
<pre class='highlight' style='line-height:initial;'><span>int main() </span><span style='background-color:#4A789C'>{                  </span>
<span style='background-color:#4A789C'>  return 0;                   </span>
...
IR for the {ref}`coverage mapping sample <coverage mapping sample>` that was shown earlier:
...
{ref}`types <cvmtypes>` like variable-length unsigned integers, that
are used to encode {ref}`File ID Mapping`, {ref}`Counter Expressions` and
the {ref}`Mapping Regions`.
...
{ref}`types <cvmtypes>` as the per-function coverage mapping data, with the
following structure:
...
String values are encoded with a {ref}`LEB value <LEB128>` for the length
...
A {ref}`coverage mapping counter` is stored in a single {ref}`LEB value <LEB128>`.
It is composed of two things --- the {ref}`tag <counter-tag>`
which is stored in the lowest 2 bits, and the {ref}`counter data` which is stored
...
The expression's kind is determined from the {ref}`tag <counter-tag>` of the
...
the {ref}`header`, which stores the counter and/or the region's kind,
and the {ref}`source range` that contains the starting and ending
...
A pseudo-counter is stored in a single {ref}`LEB value <LEB128>`, just like
