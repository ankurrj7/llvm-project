...
// RUN: llvm-profdata merge -sparse -o %t.normal.profdata %t.profraw
...
// RUN: llvm-profdata merge -sparse -o %t.d4.profdata --debug-info=%t.d4 %t.d4.proflite
...
// RUN: llvm-profdata merge -sparse -o %t.profdata --debug-info=%t %t.proflite
...
// RUN: llvm-profdata merge -sparse -o %t.cov.profdata --debug-info=%t.cov %t.cov.proflite
...
// RUN: llvm-profdata merge -sparse -o %t.cov.normal.profdata %t.cov.profraw
...
// RUN: llvm-profdata merge -sparse -o %t.normal.profdata %t-1.profraw %t-2.profraw

// RUN: rm -rf %t.profdir && mkdir %t.profdir
// RUN: env LLVM_PROFILE_FILE=%t.profdir/%m.proflite %run %t
// RUN: env LLVM_PROFILE_FILE=%t.profdir/%m.proflite %run %t
// RUN: llvm-profdata merge -sparse -o %t.profdata --debug-info=%t %t.profdir/
...
// RUN: env LLVM_PROFILE_FILE=%t.cov-1.profraw %run %t.cov.normal
// RUN: env LLVM_PROFILE_FILE=%t.cov-2.profraw %run %t.cov.normal
// RUN: llvm-profdata merge -sparse -o %t.cov.normal.profdata %t.cov-1.profraw %t.cov-2.profraw

// RUN: rm -rf %t.profdir && mkdir %t.profdir
// RUN: env LLVM_PROFILE_FILE=%t.profdir/%m.cov.proflite %run %t.cov
// RUN: env LLVM_PROFILE_FILE=%t.profdir/%m.cov.proflite %run %t.cov
// RUN: llvm-profdata merge -sparse -o %t.cov.profdata --debug-info=%t.cov %t.profdir/
