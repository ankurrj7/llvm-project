...
// RUN: llvm-profdata merge -sparse -o %t-1.profdata --binary-file=%t-1.exe %t-1.profraw
...
// RUN: llvm-profdata merge -sparse -o %t-2.profdata --binary-file=%t-1.exe %t-2.profraw
...
// RUN: llvm-profdata merge -sparse -o %t.normal.merged.profdata %t-3.profraw %t-4.profraw
...
// RUN: env LLVM_PROFILE_FILE=%t.profdir/%m-4.profraw %run %t-2.exe
// RUN: llvm-profdata merge -sparse -o %t-4.profdata --binary-file=%t-1.exe  %t.profdir
